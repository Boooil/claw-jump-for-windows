...
V2 CHANGES (permission_request support):
  This version adds handling for the "permission_request" event as the
  primary signal for user-permission-waiting state. The "notification"
  event with permission_prompt is now a fallback, not the primary trigger.

  To deploy: replace portable/agent/claw_jump_agent.py with this file.
  All other modules (cj_*.py) remain unchanged in portable/agent/.
"""
    ...
    def handle_event(payload):
        ...
        if event_name == "permission_request":
            ...
            if _should_display(payload, last_stop_by_session):
                ...
                log.info("Received permission_request (tty=%s)", tty)
                overlay.set_waiting_user("")

        elif event_name == "notification":
            if not in_session:
                return
            # If already showing waiting state (set by permission_request),
            # only update cooldown to avoid notification overriding the earlier signal.
            if overlay._state_message == "等待用户操作":
                last_stop_by_session = _update_cooldown(
                    payload, last_stop_by_session)
                _cache_context(payload)
                return
